...
import { CategoryScale, Chart, LinearScale, BarElement, Title, ArcElement, Legend} from "chart.js"
...
const RegionalData=()=> {
    ...
    const getFoodDonationState= async()=>{
        const resp= await fetch(`${REACT_APP_APIURL}/admin/getFoodDonationStates`, {
            method: "GET",
            headers: {
                "auth-token": localStorage.getItem("auth-token"),
                "Content-Type": "application/json",
    ...
    const getBlockedUsersState= async()=>{
        const resp= await fetch(`${REACT_APP_APIURL}/admin/getBlockedUsersStates`, {
            method: "GET",
            headers: {
                "auth-token": localStorage.getItem("auth-token"),
                "Content-Type": "application/json",
    ...
    return (
        ...
        <div style={{display:"flex", justifyContent:"space-evenly"}}>
            ...
            <div style={{width:"45%"}}>
                <Bar data={stateBarChartDonorData} options={stateBarChartDonorOptions}/>
            </div>
        </div>
        <div style={{display:"flex", justifyContent:"space-evenly"}}>
            <div style={{width:"45%"}}>
                <Bar data={stateBarChartFoodDonationData} options={stateBarChartFoodDonationOptions}/>
            </div>
            <div style={{width:"45%"}}>
                <Bar data={stateBarChartBlockedUsersData} options={stateBarChartBlockedUsersOptions}/>
            </div>
        </div>
        <div style={{display:"flex", justifyContent:"space-evenly"}}>
            <div style={{width:"25%"}}>
                <Pie data={stateBarChartVerifiedNgoData} options={stateBarChartVerifiedNgoOptions}/>
            </div>
        </div>
