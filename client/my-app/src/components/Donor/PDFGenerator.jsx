...
const styles = StyleSheet.create({
    ...
    donation_details:{color:'black',marginTop:25,marginLeft:50,fontSize:15},
    ...
    return  (
        ...
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.donatedItems}>
            <Text >Item: {val.name}   Quantity: {val.quantity}</Text>
          </View>
          ))}
        </Page>
      </Document>
...
const PDFGenerator = (props) =>{
   ...
   return  (
        <div>
          ...
          <PDFDownloadLink document={<MyDocument prp={props}/>} fileName={props.donor.name+'_'+props.ngo_name + '_'+props.pickUpDate+'_reciept'} style={{textDecoration:'none'}}>
            {({ blob, url, loading, error }) =>
              loading ? 'Loading document...' : 'Download Reciept'
            }
          </PDFDownloadLink>
        </div>
      );
